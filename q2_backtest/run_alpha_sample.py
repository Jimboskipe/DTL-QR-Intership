"""
Q2b — Simple alpha research sample (DTL QR intern–oriented)
...
def main() -> None:
    m = run(load(OHLC))
    lines = [
        "=== Q2b DTL alpha sample (mean-reversion + trend filter) ===",
        ...
        "Kill path: see ../q3_research/ if expectancy or DD fails.",
    ]
    text = "\n".join(lines)
    print(text)
    OUT.write_text(text + "\n", encoding="utf-8")
